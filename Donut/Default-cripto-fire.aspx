<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Default-cripto-fire.aspx.cs" Inherits="_Default" %>
...
<head runat="server">
	...
	, 'webaudiox/build/webaudiox.js'
	    ...
	    function CallMeScorePlus(value) {
	        // call server side method
	        PageMethods.genereteScorePlus(value, CallSuccessScorePlus, CallFailedScorePlus);
	    }

	    // set the destination textbox value with the ContactName
	    function CallSuccessScorePlus(res) {
	        if (res != "") {
	            score = res;
	    ...
	    // alert message on some failure
	    function CallFailedScorePlus(res) {
	        alert(res.get_message());
	    }
	        ...
	        for (l = 0; l < qtdShoot; l++) {
	            ...
	            for (li = 0; li < qtdMoon; li++) {
	                ...
	                if (distance < destroyWhenFire) {
	                    CallMeScorePlus(score);
	                    scoreShow++;
	                    //document.getElementById("scoreAspx").value = score;
	                    document.getElementById("score").innerHTML = scoreShow;
	                    resetMoon(li)
	                    resetShoot(l)
	                    reseted = true;
